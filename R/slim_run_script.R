...
#' @param remove_existing_output Should slimr remove any existing output generated by the SLiM script itself? Default is TRUE, and this is recommended if you asking slimr to collect output via the \code{output} argument, as otherwise existing SLiM output will interfere with slimr's output system.
#'
...
#'
#' @examples
slim_run_script <- function(slim_script = NULL, script_file = NULL, slim_path = NULL, output = NULL, output_every = 1, output_sample = NULL, output_format = c("genlight", "slim_tibble", "vcf_tibble"), .progress = TRUE, write_data_live = FALSE, log_output_to = NULL, subpops = "all", remove_existing_output = TRUE) {
  ...
  if(inherits(slim_script, "character") | (is.null(slim_script) & !is.null(script_file))) {
    ...
    if(inherits(slim_script, "slim_script")) {
      ...
      if(remove_existing_output) {
        slim_script <- slim_script_remove_output(slim_script)
      }
